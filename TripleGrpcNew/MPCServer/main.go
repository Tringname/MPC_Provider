...
//读函数
func readGo(Group int, Role int, Nlen int, InputDataList [100][2][]*pb.Triple) []*pb.Triple {
	mx.Lock()
	fmt.Println("在readgo出错", time.Now(), "Role=", Role, "Nlen=", Nlen)
	lockres := IsContain(Group, Role, Nlen, InputDataList)
	if !lockres {
		DataA, DataB := BeaverTriple(Nlen)
		InputDataList[Group][0] = append(InputDataList[Group][0], DataA...)
		InputDataList[Group][1] = append(InputDataList[Group][1], DataB...)
	}
	res := GetValue(Group, Role, Nlen, InputDataList)
	mx.Unlock()
	return res
}
...
func whichlock(Group int, Role int, Nlen int) []*pb.Triple {
	...
	fmt.Println("在which出错", time.Now(), "Role=", Role, "Nlen=", Nlen)

	if Group >= 0 && Role < 3 && Nlen > 0 {

		resultn = readGo(Group, Role, Nlen, IntDataList)
	} else {
...
// func whichlock2(Group int, Role int, Nlen int) []*pb.Triple {
// 	var resultn []*pb.Triple
...
//返回Map中该数据
func GetValue(Group int, Role int, Nlen int, InputDataList [100][2][]*pb.Triple) []*pb.Triple {
	if len(InputDataList[Group][Role]) >= Nlen {
		result := InputDataList[Group][Role][0:Nlen]
		InputDataList[Group][Role] = InputDataList[Group][Role][Nlen:]
		return result
	} else {
		remain := InputDataList[Group][Role][0:len(InputDataList[Group][Role])]
		InputDataList[Group][Role] = []*pb.Triple{}
		return remain
	}
}

////判断Map中是否存在该数据
func IsContain(Group int, Role int, Nlen int, InputDataList [100][2][]*pb.Triple) bool {
	fmt.Println("iscontain输入出错", InputDataList)
	if len(InputDataList) >= Group {
		if len(InputDataList[Group][Role]) >= Nlen {
			return true
		}
	} else {
		fmt.Println("iscontainelse输入出错", InputDataList)
		initdata := [2][]*pb.Triple{}
		InputDataList[Group] = initdata
		fmt.Println("iscontainelse插入输入出错", InputDataList)
		return false
	}
	return false
}
